...
const {NULL_FORK_ID} = require('../protocol/consensus');
const {encoding} = bio;
...
class CheckPacket extends Packet {
  constructor(tx, view, flags) {
    ...
    this.forkid = tx && tx.forkid ? tx.forkid : NULL_FORK_ID;
  }

  getSize() {
    return this.tx.getSize() + this.view.getSize(this.tx) + 4 + 4;
  }
  ...
  read(br) {
    ...
    this.forkid = br.readBytes(4);

    if (!this.forkid.equals(NULL_FORK_ID))
      this.tx.forkid = this.forkid;
