...
class TX extends bio.Struct {
  ...
  constructor(options) {
    ...
    // HARD FORK
    // After the hard fork gets buried under enough proof-of-work,
    // this constant can be updated "permanently" to THUMBWRESTLE_FORK_ID.
    // While activation is still in the future, it must remain null
    // and be set by the signer at the right time.
    this.forkid = null;
  ...
  inject(tx) {
    ...
    // HARD FORK
    // We do not copy the forkid into the new TX.
    // This is beacuse the forkid is only used in signing or verifying
    // and should be set by whatever calls those functions.
    // It means that MTX.toTX() will change the forkid between signing
    // in the wallet, and verifying in the chain.
  ...
  signatureHash(index, prev, value, type) {
    ...
    let size = 156 + prev.getVarSize();
    if (this.forkid)
      size += 4;
    const bw = bio.pool(size);

    if (this.forkid)
      bw.writeBytes(this.forkid);
