...
class ChainDB {
  ...
  async prove(root, key) {
    ...
    // HARD FORK
    // Restore the fork ID proof from DB, since it
    // might have been compacted away from the tree
    if (key.equals(consensus.FORK_ID_TREE_KEY)) {
      const proof = await this.db.get(layout.F.encode(root));
      if (proof)
        return UrkelProof.decode(proof, blake2b, 256);
    }
  ...
  async _saveNames(view, entry, revert) {
    ...
    // HARD FORK
    if (entry.height + 1 === this.network.thumbwrestleActivationHeight) {
      await this.txn.insert(
        consensus.FORK_ID_TREE_KEY,
        consensus.THUMBWRESTLE_FORK_ID
      );

      // Full nodes MUST save the Urkel proof for this
      // root / key / value in the DB so that even compacted-tree nodes can
      // serve it to bootstrapping light clients.
      const root = this.txn.rootHash();
      const proof = await this.txn.prove(consensus.FORK_ID_TREE_KEY);
      this.put(layout.F.encode(root), proof.encode(blake2b, 256));
    }
