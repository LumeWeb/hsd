...
class Pool extends EventEmitter {
  ...
  async _handleMerkleBlock(peer, packet) {
    ...
    if (peer.merkleBlock) {
      if (peer.pendingForkProof) {
        this.logger.warning(
          'Ignoring block %x until hard fork proof is validated (%s)',
          hash,
          peer.hostname()
        );
        return;
      ...
      this.logger.warning(
        'Peer sent merkleblock %x prematurely (%s).',
        hash,
        peer.hostname());
      peer.increaseBan(100);
      return;
    ...
    // HARD FORK
    // (spv only -- full nodes will compute 'bad-tree-root' on their own)
    if (this.chain.height === this.network.thumbwrestleActivationHeight - 1) {
      // Ask peer that sent us the block to prove that
      // the treeRoot has a valid proof for the expected FORKID.
      // If the peer does not recognize the treeRoot they will disconnect.

      // Just like waiting on TXs to fill a merkleblock,
      // we hold on to this bock until we get a proof
      ...
      this.logger.warning(
        'Requesting hard fork proof for block %x from peer (%s)',
        block.hash(),
        peer.hostname()
      );

      peer.pendingForkProof = this._resolve(
        consensus.FORK_ID_TREE_KEY,
        block.treeRoot,
        peer
      );
  ...
  async handleProof(peer, packet) {
    ...
    // HARD FORK
    if (key.equals(consensus.FORK_ID_TREE_KEY)) {
      if (!peer.merkleBlock || !peer.pendingForkProof) {
        this.logger.warning(
          'Peer sent us an unsolicited hard fork proof: %x/%x (%s)!',
          key,
          root,
          peer.hostname());
        peer.increaseBan(100);
      ...
      if (!data || !data.equals(consensus.THUMBWRESTLE_FORK_ID)) {
        this.logger.warning(
          'Invalid fork ID proof for block %x from peer (%s)',
          peer.merkleBlock.hash(),
          peer.hostname()
        );
        ...
        peer.pendingForkProof = null;
        this.logger.warning(
          'Hard fork activated: proof accepted for block %x from peer (%s)',
          peer.merkleBlock.hash(),
          peer.hostname()
        );
      }

      // If we weren't waiting on any TXs for this merkleblock,
      // we're done, send it to chain.
      if (peer.merkleMatches === 0) {
        const flags = chainCommon.flags.VERIFY_NONE;
        await this._addBlock(peer, peer.merkleBlock, flags);
  ...
  async resolve(nameHash) {
    ...
    return this._resolve(nameHash, root, peer);
  }

  /**
   * Resolve a name from a given peer at given tree root
   * @param {Buffer} nameHash
   * @param {Buffer} root
   * @param {Peer} peer
   * @returns {Buffer}
  ...
  async _resolve(nameHash, root, peer) {
    assert(Buffer.isBuffer(nameHash));
